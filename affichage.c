...
const SDL_Rect imgTerre = {11*SPRITE_WIDTH,4*SPRITE_HEIGHT, SPRITE_WIDTH, SPRITE_HEIGHT};
//Sprite imgHerbe = {.pathName = pathNameTileset, .clip = {15*SPRITE_WIDTH,1*SPRITE_HEIGHT, SPRITE_WIDTH, SPRITE_HEIGHT}, .nbImageAnim = 1}
...
const Sprite araignee = {.pathName = pathNameTileset,.clip = {9*SPRITE_WIDTH,12*SPRITE_HEIGHT, SPRITE_WIDTH, SPRITE_HEIGHT}, .nbImageAnim = 3};
const Sprite squelette = {.pathName = pathNameTileset,.clip = {6*SPRITE_WIDTH,8*SPRITE_HEIGHT, SPRITE_WIDTH, SPRITE_HEIGHT}, .nbImageAnim = 3};
//{pathNameCharset, 0,0,charset.w/4,charset.h/4,4}
const Sprite farmer = {.pathName = "Image/143-Farmer01-inaka.png", .clip = {0*SPRITE_WIDTH,0*(192/4), SPRITE_WIDTH, (192/4)}, .nbImageAnim = 4};
const Sprite yoda = {.pathName = "Image/Yoda.png", .clip = {0*SPRITE_WIDTH,0*(192/4), SPRITE_WIDTH, (192/4)}, .nbImageAnim = 4};

extern SDL_Rect* boutons;

void afficher(SDL_Surface *ecran, Jeu *game, char dplJoueur[2], int vitesse){
    /**Déclaration des variables*/
    SDL_Surface *charset =  IMG_Load(game->J1.sprite->pathName); //load le charset
    char i;
    ...
    for (i=1; i<=nbImageAnim; i++){
        ...
        apply_surface((game->J1.position[0]*nbImageAnim+dplJoueur[0]*(i-nbImageAnim))*SPRITE_WIDTH/nbImageAnim, (game->J1.position[1]*nbImageAnim+dplJoueur[1]*(i-nbImageAnim))*SPRITE_HEIGHT/nbImageAnim-game->J1.sprite->clip.h+SPRITE_HEIGHT, charset, ecran, &clip);
        /**affichage score*/
        ...
        }while(tempsActuel - tempsPrecedent < vitesse/nbImageAnim); //dpl fini en 500 ms
    }
    ...
    for (y=0;y<game->nbCaseY;y++){
        for (x=0;x<game->nbCaseX;x++){
            ...
            if(game->J1.arrivee[0] == x && game->J1.arrivee[1] == y){
                ...
                apply_text(x*SPRITE_WIDTH, y*SPRITE_HEIGHT, "A", ecran, "Fonts/OCRAStd.otf", 20, white);
            }
        }
    }
...
void afficherRessources(SDL_Surface *ecran, Jeu *game){
    ...
    for (i=0; i < game->nbRessource; i++){
        apply_surface(game->ressources[i]->position[0]*SPRITE_WIDTH, game->ressources[i]->position[1]*SPRITE_HEIGHT, textures, ecran, game->ressources[i]->image);
    }
    SDL_FreeSurface(textures); //On libère la surface
}
...
char gestionEvent(SDL_Surface *ecran, Jeu *game, Option *opt){
    /**Déclaration des variables*/
    ...
    do{SDL_WaitEvent(&event);
        if (event.type == SDL_KEYDOWN){
            switch(event.key.keysym.sym){
                case SDLK_ESCAPE:
                    choix = gestionMenu(ecran, game, opt);
                    break;
                case SDLK_r:
                    choix = rejouer;
                    break;
...
char gestionMenu(SDL_Surface *ecran, Jeu *game, Option *opt){
            ...
            case SDL_KEYDOWN:
                switch (event.key.keysym.sym){
                    ...
                    case SDLK_o:
                        ...
                        dplJoueur[0] = 0;dplJoueur[1] = 0;afficher(ecran, game, dplJoueur, 0);
                        boutons = afficherMenuRejouer(ecran);//on enleve le menu option (reaffichage du menu rejouer)
                        break;
                    default:
                        break;
                }
    ...
    if(choix!=rejouer){dplJoueur[0] = 0;dplJoueur[1] = 0;afficher(ecran, game, dplJoueur,0);}//enlever menu de lecran
                        ...
                        case SDL_MOUSEBUTTONUP:
                            if (event.button.button == SDL_BUTTON_LEFT){
                                if(event.button.x>=boutons[0].x && event.button.x<=boutons[0].x+boutons[0].w && event.button.y>=boutons[0].y && event.button.y<=boutons[0].y+boutons[0].h){
                                    ...
                                    opt->nbRessource = gestionTextBox(opt->nbRessource, ecran, boutons[3]);
                                    ...
                                    opt->nbCaseX = gestionTextBox(opt->nbCaseX, ecran, boutons[4]);
                                    ...
                                    opt->nbCaseY = gestionTextBox(opt->nbCaseY, ecran, boutons[5]);
...
int gestionTextBox(int value, SDL_Surface *ecran, SDL_Rect zone){
    ...
    do{
        /**Affichage de la valeur modifié*/
            popup = SDL_CreateRGBSurface(SDL_HWSURFACE, zone.w, zone.h, 32, 0,0,0,0);
            ...
            popup = SDL_CreateRGBSurface(SDL_HWSURFACE, texte->w, texte->h, 32, 0,0,0,0);
